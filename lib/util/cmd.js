...
var throttler = new PThrottler(1);
...
function executeCmd(command, args, options, n, preDeferred) {
    var process;
    var stderr = '';
    var stdout = '';
    var deferred = preDeferred ? preDeferred : Q.defer();
    ...
    process.on('error', function (error) {
      if (n > 0) {
        executeCmd(command, args, options, n - 1, deferred);
      }else {
    ...
    process.on('close', function (code) {
        ...
        if (code) {
            ...
            if (n > 0) {
              console.log("regry " + n + " " + command + args )
              executeCmd(command, args, options, n - 1, deferred);
            }else {
              return deferred.reject(error);
            }
        } else {
          console.log(" ok resolved " + command + args + stdout)
...
function cmd(command, args, options) {
    return throttler.enqueue(executeCmd.bind(null, command, args, options, 5));
}
